...
- [2026 年主流開源 LLM 架構圖鑑](notes/llm-architecture-map-2026.md) — 把 gemma-4（E4B / 26B-A4B）、Qwen 3.6 / 3.8、DeepSeek-V4（Flash / Pro）、Kimi K3、MiniMax M2.7 的 config 攤開對照：一覽表（層數/hidden/總參數/激活參數/注意力組成/專家配置/context）、四種長 context 策略的層堆疊圖（滑動視窗交錯、線性注意力混合、MLA 壓縮 KV、不做處理）、gemma-4 全家族（E2B/E4B/26B-A4B/31B）的旋鈕對照、Meta 的 Muse-Glimmer-30B（同尺寸選 dense 而非 MoE）、「全域層 head_dim 512 而滑動層 256」這個估 KV 時容易算錯的地方、MoE 從 128 專家 6.3% 激活走到 896 專家 1.8% 激活的演化，以及這些差異對 KV cache 成本與部署的實際影響。
